...
BEGIN
	...
	monto_total_hotel,fecha_reserva,fecha_ingreso,fecha_egreso,estado_reserva_id,viajero_id,hotel_id)

    VALUES (@monto_total,@fecha_reserva,@fecha_ingreso,@fecha_egreso,@estado_reserva_id,@viajero_id,@hotel_id);
...
BEGIN
    ...
    -- Obtener la fecha de ingreso de la reserva
    SELECT @fecha_ingreso = fecha_ingreso
    FROM Reservas_Hoteles
    WHERE id = @id_reserva;
    -- Validar si faltan al menos 2 días
    IF @fecha_ingreso IS NULL
    BEGIN
        RAISERROR('Reserva no encontrada.', 16, 1);
        RETURN;
    END
    IF DATEDIFF(DAY, GETDATE(), @fecha_ingreso) < 2
    BEGIN
        RAISERROR('No se puede cancelar la reserva con menos de 2 días de anticipación.', 16, 1);
        RETURN;
    END
    UPDATE Reservas_Hoteles
    SET estado_reserva_id = 2 -- Cancelada
    WHERE id = @id_reserva
END
...
--Procedimiento para generar el detalle de la reserva
create procedure mostrarDetalleReserva
 @id_reserva INT
AS
BEGIN
    SET NOCOUNT ON;
...
--Procedimiento para generar comprobante de cancelacion
CREATE PROCEDURE generarComprobanteCancelacion
@id_reserva int
AS
	BEGIN
SET NOCOUNT ON;
SELECT 
rh.id,monto_total_hotel,fecha_reserva,fecha_egreso,fecha_ingreso,er.descripcion_estado_reserva,h.nombre_hotel
FROM Reservas_Hoteles rh
INNER JOIN Estados_Reservas er on er.id=rh.estado_reserva_id
INNER JOIN Hoteles h on h.id=rh.hotel_id
where rh.id=@id_reserva and rh.estado_reserva_id=2
END;
